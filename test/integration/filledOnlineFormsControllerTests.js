...
describe('1. createFilledOnlineFrom', function () {
...
describe('2. displayFilledForm', function () {
        ...
        onlineFormsController.createAllOnlineForms(() => {
            filledOnlineFormsController.createFilledOnlineFrom(formName, formFields1, (err, res) => {
                if (err) done(err);
                    ...
                    let formID = res._id;
                    filledOnlineFormsController.displayFilledForm(formID, (err, locals, src) => {
                        if (err) done(err);
                        assert.equal(src, formSRC);
                        let res = JSON.parse(locals.fields);
                        for (let i = 0; i < res.length; i++) {
                            assert.equal(res[i].fieldName, formFields1[i].field);
                            assert.equal(res[i].value, formFields1[i].value);
                        }
                        assert.equal(locals.isForShow, true);
                        assert.equal(locals.formName, formName);
                        done();
                    });
                }
            })
    ...
    it('2.2 checks for invalid form', function (done) {
        this.timeout(10000);
        filledOnlineFormsController.displayFilledForm(new ObjectID(1), (err) => {
            expect(err).to.be.an('error');
            done();
        });
    });
...
describe('3. getFormReady', function () {

    before(connectsToTestingDatabase);
    afterEach(clearDatabase);
    after(closeConnection);

    it('3.1 checks for valid form when no filled happened', function (done) {
        this.timeout(10000);
    ...
    it('3.2 checks for valid form when filled happened', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    ...
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.updateOrAddFilledForm(processName, formName, formFields1, (err) => {
                            ...
                            filledOnlineFormsController.getFormReady(processName, formName, (err, form) => {
                                if (err) done(err);
                                expect(form).to.not.be.a('null');
                                expect(form).to.not.be.a('undefined');
                                done();
                            })
                        });
    ...
    it('3.3 checks for invalid form', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    if (err) done(err);
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.getFormReady(processName, "INVALID", (err, form) => {
                            expect(form).to.be.an('undefined');
                            done();
                        })
                    });
                })
    ...
    it('3.4 checks for invalid process', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    if (err) done(err);
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.getFormReady("INVALID", formName, (err, form) => {
                            expect(err).to.be.an('error');
                            done();
                        })
                    });
...
describe('4. getFormReadyToFill', function () {

    before(connectsToTestingDatabase);
    afterEach(clearDatabase);
    after(closeConnection);

    it('4.1 checks for valid form when no filled happened', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    if (err) done(err);
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.getFormReadyToFill(processName, formName, (err, src, locals) => {
                            if (err) done(err);
                            assert.equal(src, formSRC);
                            assert.equal(locals.formName, formName);
                            assert.equal(locals.fields, false);
                            assert.equal(locals.isForShow, false);
                            done();
                        })
                    })
                })
    ...
    it('4.2 checks for valid form when filled happened', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
    ...
    it('4.3 checks for invalid form', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    if (err) done(err);
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.getFormReadyToFill(processName, "INVALID", (err, form) => {
                            expect(err).to.be.an('error');
                            done();
                        })
                    });
    ...
    it('4.4 checks for invalid process', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                ...
                createActiveProcess((err) => {
                    if (err) done(err);
                    onlineFormsController.createAllOnlineForms(() => {
                        filledOnlineFormsController.getFormReadyToFill("INVALID", formName, (err, form) => {
                            expect(err).to.be.an('error');
                            done();
                        })
                    });
...
describe('5. updateOrAddFilledForm', function () {

    before(connectsToTestingDatabase);
    afterEach(clearDatabase);
    after(closeConnection);

    it('5.1 add filled form', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
                        ...
                        activeProcessController.getActiveProcessByProcessName(processName, (err, process) => {
                            ...
                            filledOnlineFormsController.updateOrAddFilledForm(processName, formName, formFields1, (err) => {
                                ...
                                activeProcessController.getActiveProcessByProcessName(processName, (err, process) => {
                                    ...
                                    filledOnlineFormsController.getFilledOnlineFormByID(process.filledOnlineForms[0], (err, res) => {
                                        if (err) done(err);
                                        else {
                                            assert.equal(res.formObject.formName, formName);
                                            for (let i = 0; i < res.formObject.fields.length; i++) {
    ...
    it('5.2 checks for update form', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
    ...
    it('5.3 checks for invalid form', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
    ...
    it('5.4 checks for invalid process', function (done) {
        this.timeout(10000);
        createUserAndRolesTree((err) => {
            if (err) done(err);
            createProcessStructure((err) => {
