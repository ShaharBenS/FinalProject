...
module.exports.startProcessByUsername = (username, processStructureName, process_name, callback) => {
    HELPER.getRoleName_by_username(username, (roleName) => {
        HELPER.getProcessStructure(processStructureName, (processStructure) => {
            HELPER.getActiveProcessByProcessName(process_name, (activeProcesses) => {
                if (activeProcesses === false)
...
module.exports.handleProcess = (username, process_name, stageDetails, filledForms, fileNames, callback) => {
    HELPER.getActiveProcessByProcessName(process_name, (process) => {
        if (process === false)
            throw ">>> ERROR: there is already process with the name: " + process_name;
